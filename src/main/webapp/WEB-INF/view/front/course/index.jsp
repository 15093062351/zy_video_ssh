...
<body class="w100">
	...
	<div id="app">
		<!--banner图-->
		<div class="banner"
			style="background-image: url('ss/img/banner-${subjectId}.jpg')"></div>
			...
			<div class="container" id="dataContainer">
				<c:forEach items="${courses }" var="course">
					<div class="section">
						...
						<div class="kcIntro">
							<p class="int">
								<span>课程介绍：</span> ${course.courseDescr }
							</p>
						</div>
						<ul>
							<c:forEach items="${course.videoset }" var="video">
								...
								<li class="section-main" onclick="getVideo(${video.id})">
									<div class="thum"
										style="background-image: url(${video.videoImageUrl})"></div>
									<p>${video.videoTitle }</p>
									<div class="classify-v-info">
										<span class="count" title="观看次数"> <img
											src="ss/img/count.png" alt="">${video.videoPlayTimes }</span>
										<span class="duration" title="视频时长"> <img
											src="ss/img/player.png" alt="">${video.videoLengthStr }</span>
									</div>
								</li>
							</c:forEach>
						</ul>
	...
	<script type="text/javascript">
		function getVideo(id){
			...
			location.href='front/videoindex?videoId='+id+'&subjectId='+subjectId;
		}
	</script>
